...
from .xyz import D65_TO_D50_ADAPTATION, rgb_to_xyz, xyz_to_rgb, xyz_to_xyy


class XyzTest(LinalgTestCase):
    def test_rgb_to_xyz(self) -> None:
        self.assert_vector_equal(rgb_to_xyz([0.0, 0.0, 0.0], SRGB), [0.0, 0.0, 0.0])
        self.assert_vector_equal(rgb_to_xyz([1.0, 1.0, 1.0], SRGB), white_point.D65_XYZ)
        self.assert_vector_equal(
            rgb_to_xyz([0.02, 0.5, 0.8], SRGB),
            [0.1861276, 0.1969824, 0.5993615],
        )
        self.assert_vector_equal(
            rgb_to_xyz([1.0, 1.0, 1.0], PROPHOTO), white_point.D50_XYZ
        )

    def test_xyz_to_rgb(self) -> None:
        self.assert_vector_equal(
            [0.02, 0.6, 1], xyz_to_rgb(rgb_to_xyz([0.02, 0.6, 1], SRGB), SRGB)
        )
        self.assert_vector_equal(
            [0.02, 0.6, 1], xyz_to_rgb(rgb_to_xyz([0.02, 0.6, 1], PROPHOTO), PROPHOTO)
        )

    def test_xyz_to_xyy(self) -> None:
        self.assert_vector_equal(xyz_to_xyy([0.0, 0.0, 0.0]), [0.95047, 1.0, 0.0])
